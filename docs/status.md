...
| ROUTE-01 | P0 | FIXED LOCALLY: versioned empty-withdrawal topology, rooted UI reachability, and accepted-only directed dispatch remove stale-hop/false-relay and false-send paths; private broadcast fallback is removed. Physical relay/reconnect validation remains open. | Stable A-B-C route/withdraw/recover run with receipts, zero-first-hop Radar check, queue rejection, no private broadcast, and no repeated callback retirement |
