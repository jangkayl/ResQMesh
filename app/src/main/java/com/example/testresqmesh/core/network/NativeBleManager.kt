...
class NativeBleManager(private val context: Context) {
    ...
    private val payloadDispatcher = PayloadDispatcher(object : PayloadDispatcherCallback {
        ...
        override fun onDeviceNameSync(endpointId: String, realName: String) {
            ...
            if (oldName == null || oldName == "Unknown" || oldName == "Unknown Peer") {
                AppLogger.d("BLE_MESH", "Name Sync: Resolving $endpointId to $realName")
                connectedEndpointNames[endpointId] = realName
    ...
    fun sendDirectPayload(targetMacAddress: String, payloadBytes: ByteArray) {
        ...
        if (isServerConnected || isClientConnected) {
            ...
            if (activeConnections.size >= MAX_CONNECTIONS) {
                ...
                val macToEvict = lruMac ?: activeConnections.keys.firstOrNull()
